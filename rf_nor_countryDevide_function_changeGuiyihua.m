clear all;
clc;warning off;
load('F:\数据\PM-nor-全国结果\B-全国rf训练好的模型\pm30Trees-有测试集散点的-longla归一化0-2');
...
test_input_array2D=zeros(norMetoNum*size(trendline,1),numpara);
...
for timei=1:size(trendline,1)
    ...
    %该时间点的气象300个随机构建完成
    test_input_array2D(norMetoNum*timei+1:norMetoNum*timei+norMetoNum,:)=test_input;
...
test_input_nor([1 2 3 4 5 6 7 10 11 12],:) = mapminmax('apply',test_input([1 2 3 4 5 6 7 10 11 12],:),ps_input);

test_input_nor([8 9],:) = mapminmax('apply',test_input([8 9],:),ps_input_lonla);
...
for timej = 1:size(trendline,1)
    nor_pm(timej)=mean(sim_aver((timej-1)*norMetoNum+1:(timej-1)*norMetoNum+300));
end

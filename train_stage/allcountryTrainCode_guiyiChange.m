...
outputIndex = [6];
%调用随机森林模型
[R2,PCC,mae,mape,rmse,rfnet,ps_input,ps_output,ps_input_lonla,trueValue,simValue,nonullDataLine,a]=rf(jjjtable2D,inputIndex, outputIndex);


%这里的a是随机数列
function [R2,PCC,mae,mape,rmse,rfnet,ps_input,ps_output,ps_input_lonla,trueValue,simValue,nonullDataLine,a]=rf(trainedData,inputIndex, outputIndex)
...
[train_input_nor([1 2 3 4 5 6 7 10 11 12],:), ps_input] = mapminmax(train_input([1 2 3 4 5 6 7 10 11 12],:),0,1);
[train_input_nor([8 9],:), ps_input_lonla] = mapminmax(train_input([8 9],:),0,2);

test_input_nor_accuracy([1 2 3 4 5 6 7 10 11 12],:) = mapminmax('apply',test_input_accuracy([1 2 3 4 5 6 7 10 11 12],:),ps_input);

test_input_nor_accuracy([8 9],:) = mapminmax('apply',test_input_accuracy([8 9],:),ps_input_lonla);

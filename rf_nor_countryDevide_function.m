clear all;
clc;warning off;
load('F:\数据\PM-nor-全国结果\B-全国rf训练好的模型\rf模型-pm30Trees-有测试集散点的.mat');
...
for timei=1:size(trendline,1)
    ...
    random300=randperm(numMetoRange,norMetoNum);
    %随机数对应的300个随即气象作为输入
    test_input=metoRange(random300,1:end-trendsize-longlaSize);
    ...
    %该时间点的气象300个随机构建完成
    test_input_array2D(norMetoNum*(timei-1)+1:norMetoNum*(timei-1)+norMetoNum,:)=test_input;
